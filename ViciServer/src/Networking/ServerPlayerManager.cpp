...
namespace Networking {
	...
	void ServerPlayerManager::sendInitialPlayerData(ENetPeer* peer, SimplePacket& packet) {
		...
		_players.emplace(peer->connectID, std::make_unique<Entities::ServerPlayer>(
			username, playerId, peer, playerData["animation"].get<std::string>(), playerData["world"].get<std::string>(),
			playerData["dir"].get<int>(), playerData["x"].get<int>(), playerData["y"].get<int>(),
			playerData["cameraZoom"].get<float>(), clientW, clientR
		));
	...
	void ServerPlayerManager::updatePlayerClientW(uint32_t id, nlohmann::json& json) {
		...
		if (!_players.contains(id)) return;
		_players.at(id)->getClientW().set<nlohmann::json&>(json["k"].get<std::string>(), json["v"]);
